...
class WGAN_GP(keras.Model) :    
    def __init__( self, generator , discriminator , latent_generator, real_examples, batch_size=200, disc_iter=2, learning_rate=.005,lambda_gp=10):
        ...
        self.generator.loss      = lambda x : - tf.math.reduce_mean(x)                                  # <---- to be filled in                 
        ...
        self.discriminator.loss      = lambda x,y : tf.math.reduce_mean(y) - tf.math.reduce_mean(x)                         # <---- to be filled in  
    ...
    def train_step(self,useless_argument):
        for _ in range(self.disc_iter) :
            ...
            gradients_discriminator = g_discr.gradient(new_discr_loss,self.discriminator.trainable_variables)
            self.discriminator.optimizer.apply_gradients(zip(gradients_discriminator, self.discriminator.trainable_variables))
        ...
        gradients_generator = g_gen.gradient(gen_loss,self.generator.trainable_variables)
